...
static NSString *const kAutoGLMMaxCompletionTokensKey = @"AUTOGLM_MAX_COMPLETION_TOKENS";
static NSString *const kAutoGLMUseCustomSystemPromptKey = @"AUTOGLM_USE_CUSTOM_SYSTEM_PROMPT";
static NSString *const kAutoGLMCustomSystemPromptKey = @"AUTOGLM_CUSTOM_SYSTEM_PROMPT";
...
static NSString *AutoGLMFormattedDateEN(void)
{
  NSDate *date = [NSDate date];
...
static NSString *AutoGLMHTMLEscape(NSString *s)
{
  NSString *out = s ?: @"";
  out = [out stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"];
  out = [out stringByReplacingOccurrencesOfString:@"<" withString:@"&lt;"];
...
static NSString *AutoGLMJSONStringLiteral(NSString *s)
{
  NSString *value = s ?: @"";
  NSError *err = nil;
...
static NSString *AutoGLMDefaultSystemPromptTemplate(void)
{
  return
	    @"今天的日期是: {{DATE_ZH}}\n"
    ...
    @"- {\"action\":\"Note\",\"message\":\"...\"}\n"
    @"    Note 是“工作记忆/笔记”（Working Notes）：\n"
    @"    - 用 message 覆盖/改写整段笔记（不是追加；你可以自由重写、整理、删除、压缩信息）。\n"
    @"    - Working Notes 会在后续每一步都提供给你（你不需要在回答里反复复述）。\n"
    @"    - 每次任务 Start 会自动清空 Working Notes。\n"
    @"    - 何时用 Note：当你从截图/页面中读到了“后续一定要用到且容易忘”的信息时，例如：要填写到表格的文字、账号/链接/ID、统计结果、已完成的收集列表等。\n"
    @"    - 不要滥用：只有当笔记需要更新时才输出 Note；否则直接输出下一步 UI 操作。\n"
    @"    - Note 不会触发任何 UI 操作，只是在你的 Working Notes 里写下信息。\n"
    @"    示例：{\"action\":\"Note\",\"message\":\"已收集封面文字：\\n1) ...\\n2) ...\"}\n"
...
{
  return AutoGLMDefaultSystemPromptTemplate();
}

- (NSString *)renderSystemPromptTemplate:(NSString *)template
{
  NSString *out = template ?: @"";
  out = [out stringByReplacingOccurrencesOfString:@"{{DATE_ZH}}" withString:AutoGLMFormattedDateZH()];
  out = [out stringByReplacingOccurrencesOfString:@"{{DATE_EN}}" withString:AutoGLMFormattedDateEN()];
  return out;
}
...
{
  NSString *payload = AutoGLMTrim(text ?: @"");
  ...
  if ([obj isKindOfClass:NSDictionary.class]) {
    ...
    if (![dict[@"action"] isKindOfClass:NSString.class]) {
      if (error) {
        *error = [NSError errorWithDomain:@"AutoGLMOnDeviceAgent" code:2 userInfo:@{NSLocalizedDescriptionKey: @"Invalid action JSON: field 'action' must be a string"}];
      }
      return nil;
    }
    return dict;
...
- (NSDictionary *)callModelWithMessages:(NSArray<NSDictionary *> *)messages error:(NSError **)error
{
  ...
  NSDictionary *body = @{
    ...
    @"messages": messages ?: @[],
    @"temperature": @1,
    @"max_completion_tokens": @(AutoGLMParseInt(self.config[kAutoGLMMaxCompletionTokensKey], 32768)),
  };
...
- (void)runLoop
{
  ...
  BOOL useCustom = AutoGLMParseBool(self.config[kAutoGLMUseCustomSystemPromptKey], NO);
  if (useCustom) {
    NSString *custom = AutoGLMStringOrEmpty(self.config[kAutoGLMCustomSystemPromptKey]);
    if (custom.length > 0) {
      systemTemplate = custom;
    }
  }
  [self.context addObject:@{@"role": @"system", @"content": [self renderSystemPromptTemplate:systemTemplate] ?: @""}];
  ...
  for (NSInteger step = 0; step < maxSteps; step++) {
    ...
    if (modelReasoning.length == 0) {
      NSString *raw = AutoGLMTrim(content);
      if (raw.length > 0) {
        [self emit:[NSString stringWithFormat:@"[AUTOGLM] Raw model content: %@", raw]];
      }
    }
...
- (void)loadConfigFromDefaults
{
  ...
  BOOL useCustomSystemPrompt = [d boolForKey:kAutoGLMUseCustomSystemPromptKey];
  NSString *customSystemPrompt = [d stringForKey:kAutoGLMCustomSystemPromptKey] ?: @"";

  NSInteger maxSteps = (NSInteger)[d integerForKey:kAutoGLMMaxStepsKey];
  NSInteger maxCompletionTokens = (NSInteger)[d integerForKey:kAutoGLMMaxCompletionTokensKey];
  ...
  self.config[kAutoGLMUseCustomSystemPromptKey] = @(useCustomSystemPrompt);
  self.config[kAutoGLMCustomSystemPromptKey] = customSystemPrompt;
  self.config[kAutoGLMMaxCompletionTokensKey] = @(maxCompletionTokens);
...
- (void)persistConfigToDefaults
{
  ...
  [d setBool:AutoGLMParseBool(self.config[kAutoGLMUseCustomSystemPromptKey], NO) forKey:kAutoGLMUseCustomSystemPromptKey];
  NSString *customSystemPrompt = AutoGLMStringOrEmpty(self.config[kAutoGLMCustomSystemPromptKey]);
  if (customSystemPrompt.length > 0) {
    [d setObject:customSystemPrompt forKey:kAutoGLMCustomSystemPromptKey];
  } else {
    [d removeObjectForKey:kAutoGLMCustomSystemPromptKey];
  }

  [d setInteger:AutoGLMParseInt(self.config[kAutoGLMMaxCompletionTokensKey], 32768) forKey:kAutoGLMMaxCompletionTokensKey];
...
- (void)updateConfigWithArguments:(NSDictionary *)args
{
  ...
  if ([args objectForKey:@"use_custom_system_prompt"] != nil) {
    self.config[kAutoGLMUseCustomSystemPromptKey] = @(AutoGLMParseBool(args[@"use_custom_system_prompt"], NO));
  }
  if ([args objectForKey:@"system_prompt"] != nil) {
    self.config[kAutoGLMCustomSystemPromptKey] = AutoGLMStringOrEmpty(args[@"system_prompt"]);
  }
  ...
  if ([args objectForKey:@"max_completion_tokens"] != nil) {
    self.config[kAutoGLMMaxCompletionTokensKey] = @(AutoGLMParseInt(args[@"max_completion_tokens"], 32768));
  }
...
- (NSDictionary *)safeConfigSnapshot
{
  ...
  cfg[@"use_custom_system_prompt"] = @(AutoGLMParseBool(self.config[kAutoGLMUseCustomSystemPromptKey], NO));
  cfg[@"system_prompt"] = AutoGLMStringOrEmpty(self.config[kAutoGLMCustomSystemPromptKey]);

  cfg[@"max_completion_tokens"] = @(AutoGLMParseInt(self.config[kAutoGLMMaxCompletionTokensKey], 32768));
...
static NSString *AutoGLMAgentPageHTML(void)
{
  ...
  @"    <input id=\"max_completion_tokens\" placeholder=\"32768\" inputmode=\"numeric\" />\n"
  @"    <h3>System Prompt</h3>\n"
  @"    <div class=\"muted\">\n"
  @"      <div>Custom system prompt only takes effect when the checkbox is enabled.</div>\n"
  @"      <div>Date placeholders: <code>{{DATE_ZH}}</code>: __DATE_ZH_EXAMPLE__ &nbsp; <code>{{DATE_EN}}</code>: __DATE_EN_EXAMPLE__</div>\n"
  @"    </div>\n"
  @"    <label class=\"check muted\"><input type=\"checkbox\" id=\"use_custom_system_prompt\" /> <span>Use custom system prompt</span></label>\n"
  @"    <textarea id=\"system_prompt\" class=\"mono\" placeholder=\"(default system prompt)\" style=\"min-height:260px\"></textarea>\n"
  ...
  @"      const ids = ['base_url','model','api_key','task','max_steps','max_completion_tokens','timeout_seconds','step_delay_seconds','system_prompt'];\n"
  @"      for (const id of ids){\n"
  ...
  @"      }\n"
  @"      const cbs = ['remember_api_key','insecure_skip_tls_verify','debug_log_raw_assistant','use_custom_system_prompt'];\n"
  @"      for (const id of cbs){\n"
  @"        const el = document.getElementById(id);\n"
  @"        if (!el) continue;\n"
  @"        el.addEventListener('change', markDirty);\n"
  ...
  @"      document.getElementById('max_completion_tokens').value = cfg.max_completion_tokens || 32768;\n"
  ...
  NSString *dateZh = AutoGLMFormattedDateZH();
  NSString *dateEn = AutoGLMFormattedDateEN();
  NSString *defaultSystemPrompt = AutoGLMDefaultSystemPromptTemplate();
  html = [html stringByReplacingOccurrencesOfString:@"__DATE_ZH_EXAMPLE__" withString:AutoGLMHTMLEscape(dateZh)];
  html = [html stringByReplacingOccurrencesOfString:@"__DATE_EN_EXAMPLE__" withString:AutoGLMHTMLEscape(dateEn)];
  html = [html stringByReplacingOccurrencesOfString:@"__DEFAULT_SYSTEM_PROMPT_JSON__" withString:AutoGLMJSONStringLiteral(defaultSystemPrompt)];
  return html;
}
