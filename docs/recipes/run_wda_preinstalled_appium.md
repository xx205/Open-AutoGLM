...
## 第 3 步：让 Open-AutoGLM 复用现有 WDA session

当 WDA 已启动后，你可以直接跑：

```bash
python ios.py --wda-url http://<iphone-ip>:8100 --wda-status
python ios.py --wda-url http://<iphone-ip>:8100 "打开 Safari 搜索 iPhone 使用技巧"
```

如果你的 WDA 只能走 USB：

1. 先用 `iproxy 8100 8100` 或 `iproxy -n 8100 8100` 把端口转发到本机；
2. 然后把 `--wda-url` 改成 `http://127.0.0.1:8100`。

Open-AutoGLM iOS 代码会在创建 session 失败时自动探测并复用已有 session（无需你手动填 session id）。
