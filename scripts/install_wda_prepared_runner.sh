...
After install:
  ...
  - Then open Safari: http://127.0.0.1:8100/autoglm
...
device=""
wda_dir=""
derived_data="$HOME/wda_derived_autoglm"
out_dir="/tmp/WDA-Prepared-AutoGLM"
do_build="1"
identity=""
...
if [[ -z "$wda_dir" ]]; then
  if [[ -d "$repo_root/../WebDriverAgent-AutoGLM" ]]; then
    wda_dir="$repo_root/../WebDriverAgent-AutoGLM"
  elif [[ -d "$repo_root/../WebDriverAgent" ]]; then
    wda_dir="$repo_root/../WebDriverAgent"
  else
    echo "Missing --wda-dir and no default ../WebDriverAgent-AutoGLM or ../WebDriverAgent found." >&2
    exit 2
  fi
fi
...
echo "  3) Open Safari: http://127.0.0.1:8100/autoglm"
